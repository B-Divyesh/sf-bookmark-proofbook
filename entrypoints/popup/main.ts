...
import './style.css';
import { exportHtml, exportJson, extractTextFromHtml, hashText, makeRecord, parseBookmarksHtml, recordsForLinkCheck, searchRecords, type BookmarkRecord } from '../../src/proofbook';
...
async function wire(query: string) {
  ...
  document.querySelector('#list')?.addEventListener('click', async (e) => {
    ...
    document.querySelector('#undo-remove')?.addEventListener('click', async () => {
      ...
      records.splice(removedRecord.index, 0, removedRecord.record); removedRecord = undefined; await store.set(records); render(query); document.querySelector('#notice')!.textContent = 'Record restored.';
    });
  });
...
async function checkLinks() {
  ...
  notice.textContent = `Checking ${pending.length} links with your explicit permission…`;
  ...
  records = [...checked, ...records.slice(pending.length)]; await store.set(records); render(); document.querySelector('#notice')!.textContent = `Checked ${checked.length} links. A changed hash means the page text differs from the saved extract.`;
}
