...
function landing() {
  ...
  APP.innerHTML = `${header()}<main id="main" tabindex="-1">
    ...
    <section class="boundary" aria-labelledby="boundary-title"><h2 id="boundary-title">What it does not do</h2><p>It does not copy full sites, bypass a paywall, or make a social feed.</p></section>
  </main>${footer()}`;
}
...
function bindGlobal() { APP.querySelectorAll<HTMLAnchorElement>('[data-nav]').forEach((a) => a.addEventListener('click', (e) => { e.preventDefault(); history.pushState({}, '', a.getAttribute('href')!); render(true); window.scrollTo(0, 0); })); }
