...
const routeMetadata = (): RouteMeta => {
  if (isDemoRoute()) return { title: 'Demo — Bookmark Proofbook', description: 'Try Bookmark Proofbook with three isolated sample bookmarks.', canonicalPath: '/demo', announcement: 'Demo — Bookmark Proofbook' };
  if (isAppRoute()) return { title: 'My proofbook — Bookmark Proofbook', description: 'Add, search, and export bookmarks in your local proofbook.', canonicalPath: '/app', announcement: 'My proofbook — Bookmark Proofbook' };
  if (location.pathname === '/privacy') return { title: 'Privacy — Bookmark Proofbook', description: 'Read how Bookmark Proofbook stores bookmarks locally.', canonicalPath: '/privacy', announcement: 'Privacy — Bookmark Proofbook' };
  if (location.pathname === '/terms') return { title: 'Terms — Bookmark Proofbook', description: 'Read the terms for using Bookmark Proofbook.', canonicalPath: '/terms', announcement: 'Terms — Bookmark Proofbook' };
  if (location.pathname !== '/') return { title: 'Page not found — Bookmark Proofbook', description: 'This Bookmark Proofbook page could not be found.', canonicalPath: '/404', announcement: 'Page not found — Bookmark Proofbook' };
  return { title: 'Bookmark Proofbook — Save bookmark context', description: 'Save why a bookmark mattered, search it later, and export a local proofbook.', canonicalPath: '/', announcement: 'Bookmark Proofbook' };
};
...
function setMetadata() {
  ...
  const canonical = new URL(meta.canonicalPath, 'https://bookmark-proofbook.sociobot.in').href;
...
function footer() { return `<footer><p>Bookmark Proofbook keeps the reason beside the link.</p><nav aria-label="Footer"><a href="/privacy" data-nav>Privacy</a><a href="/terms" data-nav>Terms</a><span>Built by Param Factory · v1.0.2</span></nav></footer>`; }

function landing() {
  setMetadata();
  APP.innerHTML = `${header()}<main id="main" tabindex="-1">
    <section class="hero" aria-labelledby="home-title"><div class="hero-copy"><p class="eyebrow">LOCAL BOOKMARK TOOL</p><h1 id="home-title">Save why each link mattered</h1><p class="lede">For people with too many bookmarks to remember, save context and find the resource again.</p><div class="actions"><a class="button moss" href="/?demo=1" data-nav>Try it with sample data</a><span>Opens a sample proofbook with three bookmarks.</span></div><ul class="facts"><li>Bookmarks are stored in this browser.</li><li>An opened proofbook works offline.</li><li>Free during this release.</li></ul><p><a class="install-jump" href="#install">Install the browser extension</a></p></div><figure><img src="/proofbook-hero.webp" srcset="/proofbook-hero-600.webp 600w, /proofbook-hero.webp 1200w" sizes="(max-width: 700px) calc(100vw - 32px), 50vw" width="1200" height="800" fetchpriority="high" decoding="async" alt="An archive box of research slips beside moss growing through concrete."/></figure></section>
...
function wireApp() {
  ...
  document.querySelector('#capture-form')?.addEventListener('submit', (e) => {
    ...
    try {
      ...
      live.unshift(record); save(); form.reset(); document.querySelector('#form-message')!.textContent = 'Saved. Its extract code is ' + record.contentHash + '.'; renderRecords();
    } catch {
  ...
  document.querySelector('#records')?.addEventListener('click', (e) => {
    ...
    if (target.classList.contains('delete')) {
      ...
      document.querySelector('#undo-remove')?.addEventListener('click', () => {
        ...
        live.splice(removedRecord.index, 0, removedRecord.record); removedRecord = undefined; save(); renderRecords((document.querySelector('#search') as HTMLInputElement).value); document.querySelector('#action-message')!.textContent = 'Bookmark restored.';
      });
    }
  });
...
function announceRoute() { const announcer = document.querySelector<HTMLElement>('#route-announcer'); if (announcer) announcer.textContent = routeMetadata().announcement; }
function render(moveFocus = false) { const demo = isDemoRoute(); if (renderedDemo && !demo) localStorage.removeItem(DEMO_STORE_KEY); if (location.pathname === '/' && !demo) landing(); else if (demo || isAppRoute()) appView(); else if (location.pathname === '/privacy' || location.pathname === '/terms') legal(location.pathname.slice(1) as 'privacy' | 'terms'); else notFound(); renderedDemo = demo; const heading = APP.querySelector<HTMLElement>('h1'); if (moveFocus && heading) { heading.tabIndex = -1; heading.focus(); announceRoute(); } bindGlobal(); }
function saveScrollPosition() { history.replaceState({ ...(history.state || {}), scrollY: window.scrollY }, '', location.href); }
function navigate(destination: string) { saveScrollPosition(); history.pushState({ scrollY: 0 }, '', destination); render(true); window.scrollTo(0, 0); }
function bindGlobal() { APP.querySelectorAll<HTMLAnchorElement>('[data-nav]').forEach((a) => a.addEventListener('click', (e) => { e.preventDefault(); navigate(a.getAttribute('href')!); })); }
...
if (!history.state?.scrollY) history.replaceState({ ...(history.state || {}), scrollY: window.scrollY }, '', location.href);
render();
