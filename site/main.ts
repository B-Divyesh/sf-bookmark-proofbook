...
function render(moveFocus = false) { if (location.pathname === '/' && !isDemoRoute()) landing(); else if (isDemoRoute() || isAppRoute()) appView(); else if (location.pathname === '/privacy' || location.pathname === '/terms') legal(location.pathname.slice(1) as 'privacy' | 'terms'); else notFound(); const heading = APP.querySelector<HTMLElement>('h1'); if (moveFocus && heading) { heading.tabIndex = -1; heading.focus(); announceRoute(); } bindGlobal(); }
function saveScrollPosition() { history.replaceState({ ...(history.state || {}), scrollY: window.scrollY }, '', location.href); }
function navigate(destination: string) { saveScrollPosition(); if (isDemoRoute() && destination !== '/demo' && destination !== '/?demo=1') localStorage.removeItem(DEMO_STORE_KEY); history.pushState({ scrollY: 0 }, '', destination); render(true); window.scrollTo(0, 0); }
